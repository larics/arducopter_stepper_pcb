...
void vTaskMotorController(void *pvParameters)
{
	...
	float P = 5;
	const int32_t mAcc = 1000;
	const int32_t mDac = 18000;
...
//	BitAction ledValue = Bit_RESET;
	...
	while(1)
	{
		...
		//Blink LED for each motor task execution
			/*switch (motorNumber)
			{
				case(1):
				{
					GPIO_WriteBit(LED_GPIOx,LED_ORANGE,ledValue);
					ledValue = (ledValue == Bit_RESET) ? Bit_SET : Bit_RESET;
					break;
				}
				case(2):
				{
					GPIO_WriteBit(LED_GPIOx,LED_BLUE,ledValue);
					ledValue = (ledValue == Bit_RESET) ? Bit_SET : Bit_RESET;
					break;
				}
				case(3):
				{
					GPIO_WriteBit(LED_GPIOx,LED_RED,ledValue);
					ledValue = (ledValue == Bit_RESET) ? Bit_SET : Bit_RESET;
					break;
				}
				case(4):
				{
					GPIO_WriteBit(LED_GPIOx,LED_YELLOW,ledValue);
					ledValue = (ledValue == Bit_RESET) ? Bit_SET : Bit_RESET;
					break;
				}
			}*/
...
void TIM4_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(TIM4,TIM_IT_CC1) != RESET)
	{
		...
		if(delta != -1)
		{
			...
			//GPIO_WriteBit(LED_GPIOx,LED_ORANGE,bitValue);
	...
	if(TIM_GetITStatus(TIM4,TIM_IT_CC2) != RESET)
	{
		...
		if(delta2 != -1)
		{
			...
			//GPIO_WriteBit(LED_GPIOx,LED_BLUE,bitValue2);
	...
	if(TIM_GetITStatus(TIM4,TIM_IT_CC3) != RESET)
	{
		...
		if(delta3 != -1)
		{
			...
			//GPIO_WriteBit(GPIOD , GPIO_Pin_13, bitValue);	
			//GPIO_WriteBit(LED_GPIOx,LED_RED,bitValue3);
	...
	if(TIM_GetITStatus(TIM4,TIM_IT_CC4) != RESET)
	{
		...
		if(delta4 != -1)
		{
			...
			//GPIO_WriteBit(GPIOD , GPIO_Pin_13, bitValue);	
			//GPIO_WriteBit(LED_GPIOx,LED_YELLOW,bitValue4);
